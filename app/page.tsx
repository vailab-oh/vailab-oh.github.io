  ...
  return (
    <PageFrame>
      ...
      <section className="section-shell content-section publication-preview">
        ...
        <div className="featured-publications">{latestPublications.map((item) => <article key={item.id}><div className="featured-meta"><span>{item.year}</span><span>{item.venue}</span></div><h3>{item.url ? <a href={item.url} target="_blank" rel="noreferrer">{item.title}</a> : item.title}</h3><p className="featured-authors">{item.authors}</p><p className="featured-links">{item.url && <a className="publication-resource-link" href={item.url} target="_blank" rel="noreferrer">Paper</a>}{item.dbUrl && <><span>·</span><a className="publication-resource-link" href={item.dbUrl} target="_blank" rel="noreferrer">DB</a></>}{item.codeUrl && <><span>·</span><a className="publication-resource-link" href={item.codeUrl} target="_blank" rel="noreferrer">Code</a></>}</p></article>)}</div>
      </section>
